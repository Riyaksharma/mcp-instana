    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=timeout_seconds) as client:
        try:
            ...
            logger.error(f"❌ OAuth refresh request failed: {exc}")
            raise RuntimeError("Failed to refresh OAuth access token") from exc

    token_payload = response.json()
    logger.info(f"📦 Token Response Keys: {list(token_payload.keys())}")
    
    access_token = token_payload.get("access_token") or token_payload.get("id_token")
    if not access_token:
        logger.error(f"❌ OAuth refresh response missing access token: {token_payload}")
        raise RuntimeError("OAuth provider did not return an access token")

    logger.info(f"✅ New Access Token Received: {access_token[:20]}...")
    logger.info("=" * 80)
    return access_token
...
async def build_oauth_client(base_url: str, auth_config: Dict[str, Any]) -> httpx.AsyncClient:
    ...
    Supported auth_config keys (both camelCase and snake_case accepted):
      ...
      - tokenUrl | token_url
      - scope
      - refreshToken | refresh_token: if value looks like an ENV var key (e.g. "ENV_*"),
        the actual token will be read from that environment variable.
    """
    # Accept both naming styles
    client_id = resolve_env_value(auth_config.get("client_id") or auth_config.get("clientId"))
    client_secret = resolve_env_value(auth_config.get("client_secret") or auth_config.get("clientSecret"))
    token_url = resolve_env_value(auth_config.get("token_url") or auth_config.get("tokenUrl"))
    scope = auth_config.get("scope")

    refresh_token_value = resolve_env_value(auth_config.get("refresh_token") or auth_config.get("refreshToken"))
...
class OAuthRefreshClient(httpx.AsyncClient):
    ...
    async def _refresh_token(self) -> None:
        logger.info("🔄 OAuthRefreshClient: Refreshing token...")
        ...
        logger.info(f"✅ Token refreshed, expires at: {self._expires_at}")
